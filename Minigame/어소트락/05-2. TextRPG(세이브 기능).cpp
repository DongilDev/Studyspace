...
bool LoadPlayer(_tagPlayer* pPlayer)
{
	...
	{		
		// 플레이어 이름을 읽어온다.
		fread(pPlayer->strName, NAME_SIZE, 1, pFile);
		
		// 직업 정보를 읽어온다.
		fread(pPlayer->strJobName, NAME_SIZE, 1, pFile);
		...
		// 만약 저장할때 무기를 차고 있었다면 해당 무기정보도 같이 저장이 되었다.
...
bool SavePlayer(_tagPlayer* pPlayer)
{
	...
	{
		// 플레이어 이름을 저장한다.
		fwrite(pPlayer->strName, NAME_SIZE, 1, pFile);

		// 직업 정보를 저장한다.
		fwrite(pPlayer->strJobName, NAME_SIZE, 1, pFile);
...
int main()
{
	...
	while (iGameMode < 1 || GM_NONE || iGameMode > GM_END)
	{
		system("cls");
		cout << "1. 새로하기" << endl;
		cout << "2. 이어하기" << endl;
